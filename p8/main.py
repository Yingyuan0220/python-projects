...
def make_namedtuple_list(jdata):
    car_list = []
    Car = namedtuple("Car", ["id", "make", "model", "year", "transmission"])
    for i in jdata:
        car_namedtuple = Car(i, get_value(jdata[i], "Make"), get_value(jdata[i], "Model"), get_value(jdata[i], "Year"),
                             get_value(jdata[i], "Transmission"))
        car_list.append(car_namedtuple)
    return car_list
...
def filter_cars(cars, filters):
    ...
    for j in cars:
        if 'make' in filters:
            if filters['make'] != j.make:
                continue
        if 'model' in filters:
            if filters['model'] != j.model:
                continue
        if 'year' in filters:
            if filters['year'] != j.year:
                continue
        if 'transmission' in filters:
            if filters['transmission'] != j.transmission:
                continue
        car_list.append(j)
